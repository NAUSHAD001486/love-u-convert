...
const CLOUDINARY_UPLOAD_TIMEOUT_MS = 120000; // 120 seconds
/**
...
const uploadImageStream = (stream, options) => {
    return new Promise((resolve, reject) => {
        ...
        timeoutId = setTimeout(() => {
            if (!resolved) {
                safeReject(new Error('Cloudinary upload timeout: operation exceeded 120 seconds'));
            }
        }, CLOUDINARY_UPLOAD_TIMEOUT_MS);
        const byteTracker = new stream_1.Transform({
            transform(chunk, encoding, callback) {
                if (uploadAborted || resolved) {
                    return callback();
                }
                bytesUploaded += chunk.length;
                const newQuotaUsed = quotaUsed + bytesUploaded;
                if (quotaLimit > 0 && newQuotaUsed > quotaLimit) {
                    safeReject(new errors_1.QuotaExceededError('Daily upload limit exceeded'));
                    return callback();
                }
                callback(null, chunk);
            },
        ...
        cloudinaryUploadStream = cloudinary_1.default.uploader.upload_stream(uploadOptions, (error, result) => {
            ...
            if (!result || !result.secure_url || !result.public_id) {
                safeReject(new Error('Cloudinary upload failed: invalid response'));
                return;
            }
            safeResolve({
        ...
        // Handle cloudinaryUploadStream errors
        ...
        // Handle byteTracker errors
        byteTracker.on('error', (error) => {
            if (!resolved) {
                safeReject(error);
            }
        });
        // Pipe stream through byteTracker to cloudinaryUploadStream
