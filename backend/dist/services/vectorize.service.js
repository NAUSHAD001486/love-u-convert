 ...
 */
const vectorizeSvgLookColorPro = async (inputPath) => {
    ...
    const processedImage = (0, sharp_1.default)(inputPath)
        .resize({
 ...
 */
const vectorizeToSvg = async (inputPath, outputPath) => {
    try {
        // Step 0: Validate input (fail fast - file size only, NO dimension check)
        const fileStats = (0, fs_1.statSync)(inputPath);
        ...
        (0, fs_1.writeFileSync)(outputPath, svgString, 'utf8');
    }
    catch (error) {
...
exports.vectorizeToSvg = vectorizeToSvg;
