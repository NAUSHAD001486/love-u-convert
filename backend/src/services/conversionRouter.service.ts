...
export interface ConversionContext {
  inputFormat: string | null; // Detected from MIME/magic bytes
  targetFormat: string;
 ...
 */
const extractFormatFromMime = (mime: string | null): string | null => {
  if (!mime) return null;
  if (mime.includes('/')) {
 ...
 */
export const determineConversionRoute = (context: ConversionContext): ConversionRoute => {
...
export const executeConversion = async (
  ...
  publicId: string
): Promise<{ secure_url: string; public_id: string; bytesUploaded: number }> => {
  const route = determineConversionRoute(context);
  const targetFormat = context.targetFormat.toLowerCase();
  ...
  if (route === 'svg-to-svg') {
    ...
    return uploadImageStream(fileStream, {
      targetFormat: 'svg',
      publicId,
